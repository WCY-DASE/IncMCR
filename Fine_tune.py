...
def Incremental_update(model, span, train_data, train_task_num, device=torch.device('cpu')):
    ...
    # print("****************** Saving model!************************")
    saved_model_filename = "saved/{}/Fine_tune/{}/MeLU_Incrental_{}.pkl".format(config["dataset"],'trail2',span)  # 保存模型参数
    check_and_create_path(saved_model_filename)
    saved_model = model.state_dict()
    # print(saved_model.keys())
    torch.save(saved_model, saved_model_filename)
...
def Incremetal_train(model, config, device=torch.device('cpu')):
    ...
    for span_i in time_span_list:
        ...
        data, num_tasks = data_loader.load_data(state="Incremental_{}".format(span_i))  # 加载meta-test 数据
        print("Incremental_Span: {}, New tasks: {}".format(span_i, num_tasks))

        ### 测试
        mae_s, rmse_s, ndcg_at_3_s, ndcg_at_5_s, ndcg_at_10_s = test(model, data, device=device)
        ...
        print("增量时段: {} 增量训练时间为：{}".format(span_i, num_tasks, elapsed_time_i), "秒")
