...
def training(model, config, device=torch.device('cpu')):
    ...
    for epoch_i in range(num_epoch):
        ...
        for batch_i in tqdm(range(num_batch)):
            ...
            _loss, mae_i, rmse_i, ndcg_3_i, ndcg_5_i, ndcg_10_i = model.global_update(train_tasks_bi,
                                                                                      device=device)  # 每个batch上进行一次全局更新
        ...
        print(
            'epoch: {}, loss: {:.6f}, mae: {:.5f}, rmse: {:.5f},  ndcg_3: {:.5f}, ndcg_5: {:.5f}, ndcg_10: {:.5f}'.format(
...
def Incremental_update(model, span, new_data, replay_data, device=torch.device('cpu')):
    if config['use_cuda']:
        model.cuda()
    model.eval()  # eval模式
    ...
    num_batch_new = int(len(new_data) / batch_size)
    num_batch_replay = int(len(replay_data) / batch_size)
    ...
    for epoch_i in range(num_epoch):
        ...
        for batch_i in tqdm(range(num_batch_new)):
            train_tasks_bi = new_data[batch_size * batch_i: batch_size * (batch_i + 1)]
            _loss, mae_i, rmse_i, ndcg_3_i, ndcg_5_i, ndcg_10_i = model.global_update(train_tasks_bi,
                                                                                      device=device)  # 每个batch上进行一次全局更新
        ...
        # replay tasks update
        for batch_i in tqdm(range(num_batch_replay)):
            train_tasks_bi = replay_data[batch_size * batch_i: batch_size * (batch_i + 1)]
            _loss, mae_i, rmse_i, ndcg_3_i, ndcg_5_i, ndcg_10_i = model.global_update(train_tasks_bi,device=device, for_replay=True)  # 每个batch上进行一次全局更新
        ...
        # model.scheduler.step()
        print(
            'Train epoch: {}, loss: {:.6f}, mae: {:.5f}, rmse: {:.5f},  ndcg_3: {:.5f}, ndcg_5: {:.5f}, ndcg_10: {:.5f}'.format(
                epoch_i, np.mean(loss), np.mean(mae), np.mean(rmse), np.mean(ndcg_at_3), np.mean(ndcg_at_5),
                np.mean(ndcg_at_10)))  # report整个epoch的结果，为meat-training数据上的损失和评价参数

    # print("****************** Saving model!************************")
    saved_model_filename = "saved/{}/DCMR_Fine_tune/{}/MeLU_Incrental_{}.pkl".format(config["dataset"], 'trail1',
                                                                                       span)  # 保存模型参数
    check_and_create_path(saved_model_filename)
    saved_model = model.state_dict()
    # print(saved_model.keys())
...
def Incremetal_train(model, config, device=torch.device('cpu')):
    ...
    for span_i in time_span_list:
        ...
        Incremental_update(model, span_i, new_data, replay_data, device=device)
        old_data = new_data + replay_data
